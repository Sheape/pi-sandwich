...
function render(current: PipelineState): void {
  ...
  const frame = [
    ...
    line(
      "Input",
      `${current.input.tool}; ${current.input.byteLength} bytes; isError=${current.input.isError}`,
    ),
    ...
    line("Lossy owner", current.owner),
    line("Classification", current.classification),
    line("Evidence", current.evidence),
    line("Candidate", current.candidate),
    line("Validation", current.validation),
